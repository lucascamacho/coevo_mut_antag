...
for(k in 1:length(redes)){ # loop to each matrix of interactions
  ...
  for(a in 1:length(antprob_vec)){ # 100 loops to each matrix
    ...
    for(q in 1:length(bar_vec)){ # 5 simulations per p value
...
plot_nest = ggplot(data = new_data, aes(x = antprob, y = mean_nest_coevo)) +
  geom_point(aes(color = bar, fill = bar), alpha = 0.5,size = 2,) +
  xlab("Frequency of cheaters interactions (p)") + ylab("Delta nestedness") +
  scale_x_continuous(limits = c(0,1.1), expand = c(0,0)) +
  theme_classic()

plot_mod = ggplot(data = new_data, aes(x = antprob, y = mean_mod_coevo)) +
  geom_point(aes(color = bar, fill = bar), alpha = 0.5,size = 2,) +
  xlab("Frequency of cheaters interactions (p)") + ylab("Delta modularity") +
  scale_x_continuous(limits = c(0,1.1), expand = c(0,0)) +
  theme_classic()

plot_final = grid.arrange(plot_mod, plot_nest, nrow = 2)

ggsave(plot_final, filename = "Major_Sensibility_b.png", dpi = 600,
