...
Despite the fact that these studies helps the creation of a theoretical map of how coevolution act in traits of species and how interactions change the coevolutionary process, there is compelling evidence that ecological interactions are context dependent in nature [@ushio_fluctuating_2018, @kolb_ecological_2007, @bronstein_conditional_1994]. The interactions are context-dependet if the outcomes or the magnitude change depending on the biotic of abiotic context [@chamberlain_how_2014]. One simple example is the insect-plant interaction between the fly *Moegistorhynchus longirostri* and the tubular flower *Lapeirousia anceps*, where the fly has a long proboscis to pollinate the flower. But, the proboscis length varies geographically, resulting in efficient pollination in one area and unnefficient pollination in other. In other words, the costs and benefits of fly`s visits are changing [@pauw_flies_2009]. Using networks only with mutualism or antagonism is a convenction to disentangle the effects of these types of interactions in evolutionary process, but in real communities, species are experiencing mutualistic and antagonistic interactions at the same time. More than that, ecological interactions are not static in nature, but variable in space and time. In this way, we are trying to take a step further in coevolutinary studies, merging the evolutionary effects of antagonisms and mutalisms in the same network of ecological interactions. Here we gonna answer 2 questions about networks of mutualism and antagonism:

1. How is it the coevolutionary dynamics in these networks?
2. What is the effect of network structure in coevolutionary dynamics?
...
$$Z_{ i } ^ { (t+1) } = Z_{ i } ^ { t } + \varphi_{ i }[\sum_{j} m_{ij}^{t}(Z_{j}^{t} - Z_{i}^{t}) + \sum_{j}v_{ij}^{t}(Z_{j}^{t} + \epsilon_{ij} - Z_{i}^{t}) + p_{i}(\theta_{i} - Z_{i}^{t})]$$
...
In this component we have $m_{ij}^{t}$ that is the mutualistic evolutionary effects of a species $j$ to the specie $i$. These component favours the trait matching between $Z_{i}$ and $Z_{j}$. In other words, these values $Z_{i}$ and $Z_{j}$ tend to become more similar.
The second component is the antagonist component

$$\sum_{j}v_{ij}^{t}(Z_{j}^{t} + \epsilon_{ij} - Z_{i}^{t})$$

Similar to mutualism, we also have an evolutionary antagonistic effect of $j$ in $i$ which is $v_{ij}^{t}$. Note that we have a parameter $\epsilon$, which is our "trait barrier" or the maximum differente of $Z_{i}$ and $Z_{j}$ so that antagonistic interaction can exist. Finally, we have the environment component

$$p_{i}(\theta_{i} - Z_{i}^{t})$$

which $\theta_{i}$ are the vector with the environmental optima for each specie.
...
# importance of invironment to changes in Z
p = 0.01 
...
```{r}
  # create a matrix rolling the dice to every interaction
  P = matrix(runif(n_sp*n_sp, min = 0, max = 1),
             nrow = n_sp, ncol = n_sp) 
  # 1 in the V matrix if the number sort is lower than antprob
  V[antprob >= P] = 1 
  # 0 in the M matrix if the number sort is lower than antprob
  M[antprob >= P] = 0 
  # diagonal of v is zero (we are not interested in effects of a species in herself)
  diag(V) <- 0 
  # no intraespecifi antagonistic interactions
  diag(V) <- 0 
```
...
First, when we transform an interaction in antagonism we are defining an explorer and a victim. In these scenario, we introduce an antagonistic effect of $j$ in $i$, but not necessarily an antagonistic effect of $i$ in $j$. The V matrix tell us the antagonistic interactions of species in the collums to the species in the rows. In our V matrix, there are a antagonistic effect of specie 1 on specie 2 but not otherwise.
Second (and more intuitive), the V and M matrix are complementar. The sum of V and M gives us all the interactions in the network.
...
Ok, now we calculate the $e^{-\alpha(Z_{j}^{(t)} - Z_{i}^{(t)})^{2}}$ part using the trait differences that we already have. We are not interested in intraespecific interactions, so we gonna insert "0" in the diagonal of the matrix
...
Of course, maybe these equilibrium state doesn't happend, so the simulation will go until we reach the maximum time of simulations (t_max)
...
You can find the R code in our [GITHUB](https://github.com/lucascamacho/coevo_mut_antag) repository, in the CoevoMutAntag.R file.

I really hope these guide helps the understanding of the code. I also hope that we can schedule a Skype meeting to talk about next steps of our research.

Thank`s a lot for your time reading these guide.
