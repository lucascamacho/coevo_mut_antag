CentralAntagonize = function(M){
  ...
  # transform all the central species interactions in negative
  # and count the number of negative effects in the network.
  #
  ...
  # change interactions of central species only
  ...
  # create and return a list with the positive and negative effects matrices
  mats = list(M, V, p_vl)
  return(mats)
}
#---------------------------------------------------------------------------------------------------#